...
public class BoardController {
    ...
    public String boardMain(Model model, @RequestParam(name = "pageNo", required = false) String pageNo) {
        int pageSize = 10; // 페이지 크기 설정 (한 페이지에 보여줄 회원 수)
        int pageNum = 1;
        try {
            if (pageNo != null) {
...
//	        }
        ...
        BoardDtoPage boardDtoPage = getBoardDtoPage(pageNum, pageSize); // 사용자 페이지 정보를 조회
        int getTotal = boardService.boardAllCount();
        System.out.println("getTotal 값 : " + getTotal);
        model.addAttribute("listPage", boardDtoPage);
